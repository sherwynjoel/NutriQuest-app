@echo off
echo ========================================
echo    NutriQuest Simple App Launcher
echo ========================================
echo.

cd /d "C:\Users\Sherwyn joel\OneDrive\Desktop\NutriQuest"

echo Starting NutriQuest Simple App (No Firebase)...
echo.

REM Try to run the simple Flutter app
echo Attempting to run Flutter simple app...
flutter run lib/main_simple.dart -d chrome --web-port=3000 >nul 2>&1

if %errorlevel% equ 0 (
    echo ✅ Flutter simple app started successfully!
    echo Opening browser...
    start http://localhost:3000
) else (
    echo ❌ Flutter not available. Starting alternative demo...
    echo.
    
    REM Start Python server as fallback
    echo Starting Python server...
    start /b python -m http.server 3000
    
    REM Wait for server to start
    timeout /t 3 /nobreak >nul
    
    REM Open browser
    start http://localhost:3000/nutriquest_quiz.html
    
    echo.
    echo ✅ Alternative demo started!
    echo Working quiz available at: http://localhost:3000/nutriquest_quiz.html
)

echo.
pause
